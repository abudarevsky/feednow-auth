...
first consumers, and Phase 06 delivered the second adapter (DynamoDB) passing
the same suite against DynamoDB Local.
...
| Module | Surface | First consumer |
| --- | --- | --- |
| `src/app/storage/contract.py` | `Storage` (`@runtime_checkable` Protocol, 19 sync methods: users, external identities, organizations, memberships, API keys, `append_audit_event`, `provision_user`, `provision_organization`, three paginated lists); domain errors `StorageError`/`EntityNotFoundError`/`DuplicateEntityError` (+`DuplicateEntityKind`)/`DuplicateExternalIdentityError`/`ReferenceNotFoundError`/`InvalidCursorError`; frozen `ProvisionedUser`/`ProvisionedOrganization` caller-echo bundles | Phases 03/04/05 services; both adapters |
| `src/app/storage/__init__.py` | Contract symbols only; never imports an adapter | any `app.storage` importer |
| `src/app/storage/sqlite.py` | `open_sqlite_storage(path: str | Path) -> Storage` factory, `SQLiteStorage` adapter, schema v1 (6 tables, 5 unique indexes), codecs, opaque keyset cursors, sqlite3→domain error translation | Phase 03 provisioning; the behavior reference the DynamoDB adapter mirrors |
| `src/tests/storage_contract/suite.py` | 60 adapter-neutral behavior cases + deterministic `make_*` builders; module docstring is the fixture/import/builder reuse contract | Both conformance entries (imported unchanged; byte-identical hash pin) |
| `src/tests/storage_contract/test_sqlite_contract.py` | SQLite entry point: `storage` fixture (fresh temp file per test) + suite-import isolation proofs | Pattern copied by the DynamoDB entry (Phase 06) |

### Storage conventions

| Topic | Rule |
| --- | --- |
| Access | Application code is typed against `Storage` only and obtains instances through the documented factory; adapters are imported by explicit submodule path (`from app.storage.sqlite import open_sqlite_storage`, `from app.storage.dynamodb import open_dynamodb_storage`), never constructed above `app/storage`. |
...
| Tenant normalization | `provider_tenant=None` stores as `''` and reads back as `None` (lossless: `ProviderTenant` pins `min_length=1`); SQLite `UNIQUE` treats NULLs as distinct, so normalization is what makes the constraint deterministic. The DynamoDB adapter keeps the same semantics (normalized into the constraint key). |
...
| Referential integrity | `identity→user`, `membership→org+user`, `api_key→org+creator`, `audit→org` are enforced by every adapter (`PRAGMA foreign_keys=ON` in SQLite; `ConditionCheck`s inside the DynamoDB transaction) and raise `ReferenceNotFoundError`. |
...
| Conformance invocation | `uv run pytest src/tests/storage_contract` — SQLite entry (64: 60 suite cases + 4 entry-local proofs) always; DynamoDB entry (63: the same 60 + 3 entry-local proofs) when `FEEDNOW_DYNAMODB_LOCAL_ENDPOINT` is set and reachable, else marker-skips. `suite.py` is byte-identical across entries (hash pin). |
...
| `Storage.provision_organization(*, organization, membership, audit_events) -> ProvisionedOrganization` | `src/app/storage/contract.py` | Phase 06 replicated the atomics (one `TransactWriteItems`; suite cases are adapter-neutral) |
...
## Phase 06 DynamoDB adapter (handoff)

Phase 06 ships the second storage adapter behind the frozen 19-method
contract: the multi-table `SCHEMA` (7 tables + 3 GSIs + the key-only
`unique_constraints` table), transactional compounds with positional
conflict classification, opaque keyset cursors over GSI queries, the
conditional-update revoke CAS, and the marker-gated conformance entry
against DynamoDB Local. The full schema, least-privilege IAM matrix,
translation summary, and verification evidence live in
[docs/phases/06-dynamodb.md](docs/phases/06-dynamodb.md). Phase 06 changes
no contract/model/service/API surface and adds one runtime dependency
(`boto3>=1.35,<2`).
...
| Module | Surface | First consumer |
| --- | --- | --- |
| `src/app/storage/dynamodb.py` | `SCHEMA` (`TableSpec`/`IndexSpec`, `create_parameters`), `open_dynamodb_storage(...)` factory, `DynamoDbStorage` (19 ops + `close()`), codecs, pure positional cancellation classifier | Phase 07 deployment entrypoint + CDK; Local harness |
| `src/tests/support/dynamodb_local.py` | Endpoint probe/skip reason, `create_tables`/`delete_tables` from `SCHEMA`, `make_dynamodb_storage` (dummy credentials, per-test prefix) | Both DynamoDB test entries |
| `src/tests/storage_contract/test_dynamodb_contract.py` | DynamoDB conformance entry: `storage` fixture + `suite.py` re-exported byte-identical (hash pin) | Phase 08+ adapter hardening runs |

### Adapter conventions

| Topic | Rule |
| --- | --- |
| Boundary | `storage/dynamodb.py` is the only `src/app` module importing `boto3`/`botocore` (AST-proven); `app.main` loads no driver (subprocess-proven); `app/storage/__init__.py` stays contract-only; the adapter is imported by explicit submodule path. |
| Gating | DynamoDB Local tests carry the `dynamodb_local` marker and skip with an explicit reason unless `FEEDNOW_DYNAMODB_LOCAL_ENDPOINT` is set and reachable — the default suite needs no Docker. |
| Uniqueness | Native `attribute_not_exists` on record-id/membership-pair keys; everything else is a `unique_constraints` item (`user_email`, `organization_slug`, `external_identity`, `api_key_id`, `membership_id` guard → `entity_id`). |
| Conflicts | First `ConditionalCheckFailed` in submission order decides the domain error; transient `TransactionConflict` is retried bounded; throughput/unknown → base `StorageError`; driver text never propagates. |
| Parity | The shared suite is the oracle: a failing case changes the adapter, never `suite.py` (byte-identical hash pin across entries). |
